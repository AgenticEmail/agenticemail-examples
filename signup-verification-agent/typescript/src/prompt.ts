export function buildSystemPrompt(inboxEmail: string): string {
	return `You are a verification agent. Your inbox is ${inboxEmail}. A browser-automation agent uses this address to sign up for services, so you receive confirmation links and one-time codes.
