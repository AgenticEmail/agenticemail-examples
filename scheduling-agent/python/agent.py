"""Scheduling agent: books time over email, following your rules.

Give an AgenticEmail inbox to Claude. When someone emails asking to book time,
Claude offers slots that fit the rules and confirms a meeting - all in-thread.
"""
...
INBOX_USERNAME = os.environ.get("INBOX_USERNAME", "scheduling-agent")
...
system_prompt = build_system_prompt(inbox_email, USER_NAME, TIMEZONE)
...
def main() -> None:
    print(f"Scheduling agent inbox: {inbox_email}")
