 ...
 *
 * Give an AgenticEmail inbox to Claude. When someone emails asking to book
 * time, Claude offers slots that fit the rules and confirms - all in-thread.
 */
...
const INBOX_USERNAME = process.env.INBOX_USERNAME ?? "scheduling-agent";
...
const systemPrompt = buildSystemPrompt(inboxEmail, USER_NAME, TIMEZONE);
...
console.log(`Scheduling agent inbox: ${inboxEmail}`);
